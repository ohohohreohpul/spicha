...
function Reveal({
  ...
  // Self-animated on mount — variant propagation from a parent container is
  // not reliable across React 19 hydration, and a stranded hidden variant
  // means invisible content.
  return (
    <motion.div
...
function FeatureCard({
  ...
  return (
    ...
    >
      {card.image && (
        // eslint-disable-next-line @next/next/no-img-element -- third-party URLs stay unoptimized; next/image would need remotePatterns host config per source.
        <img
          src={card.image}
          alt={card.imageAlt ?? ''}
          decoding="async"
