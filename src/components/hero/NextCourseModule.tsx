  ...
  return (
    <div className={shell}>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        >
          {isBookable(session.status)
            ? t.nextCourse.request
            : t.nextCourse.waitlist}
        </a>
        <p className={`numeric text-xs ${muted}`}>
          {stale ? t.nextCourse.lastConfirmed : t.nextCourse.updated}{" "}
          {syncedAtLabel} {t.nextCourse.clock}
        </p>
      </div>
    </div>
  );
