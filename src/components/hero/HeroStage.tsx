  ...
  useEffect(() => {
    ...
    mm.add(MOTION_OK, () =>
      whenVisible(() => {
        ...
        if (film) tl.from(film, { scale: 1.18, duration: 2.8, ease: 'outQuint' }, 0);
        tl.from(qq(el, 'rule'), { scaleX: 0, duration: 1.1, stagger: 0.1 }, 0.1);
        tl.from(lines(q(el, 'kicker')), { yPercent: 130, duration: 0.85 }, 0.16);
        if (headlineLines.length)
          tl.from(headlineLines, { yPercent: 118, duration: 1.35, stagger: 0.085 }, 0.24);
        if (proseLines.length)
          tl.from(proseLines, { yPercent: 105, duration: 1.0, stagger: 0.045 }, 0.66);
        ...
        if (panel) tl.from(panel, { clipPath: 'inset(0% 0% 100% 0%)', y: 34, duration: 1.25 }, 0.7);
